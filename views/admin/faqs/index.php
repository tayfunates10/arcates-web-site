...
<div class="panel__head">
  <p class="muted"><?= Security::e((string) count($faqs)) ?> kayit</p>
        ...
        <tr>
          <td>
            <a href="<?= Security::e(admin_url('sss/' . (int) $row['id'])) ?>">
              <?= Security::e($row['question'] ?? '(ceviri yok)') ?>
            </a>
          </td>
          ...
          <td>
            <span class="tag tag--<?= (int) $row['on_home'] > 0 ? 'ok' : 'draft' ?>">
              <?= (int) $row['on_home'] > 0 ? 'Evet' : 'Hayir' ?>
            </span>
          </td>
          <td>
            <span class="tag tag--<?= (int) $row['status'] === 1 ? 'ok' : 'off' ?>">
              <?= (int) $row['status'] === 1 ? 'Etkin' : 'Kapali' ?>
            </span>
          </td>
          <td class="row-actions">
            <a class="btn btn--ghost btn--sm" href="<?= Security::e(admin_url('sss/' . (int) $row['id'])) ?>">Duzenle</a>
