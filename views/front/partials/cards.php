...
/** Satir ici SVG ikon kumesi; harici istek yapilmaz. DOCS.md 2 */
$icons = [
  ...
  <div class="wrap">
    ...
    <ul class="cards">
      <?php foreach ($cards as $index => $card): ?>
        <?php
        $colour = in_array((string) ($card['color'] ?? ''), $colours, true)
            ? (string) $card['color']
            : $colours[$index % count($colours)];
        $iconKey = (string) ($card['icon'] ?? '');
        $icon    = $icons[$iconKey] ?? $icons['layout'];
        ?>
        <li class="card card--<?= Security::e($colour) ?>" data-reveal>
          <span class="card__icon" aria-hidden="true">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"
                 stroke-linecap="round" stroke-linejoin="round" focusable="false"><?= $icon ?></svg>
          </span>

          <h3 class="card__title">
            <?php if (($card['url'] ?? '') !== ''): ?>
              <a href="<?= Security::e(url((string) $card['url'])) ?>"><?= Security::e($card['title'] ?? '') ?></a>
            <?php else: ?>
          ...
          <?php if (($card['text'] ?? '') !== ''): ?>
            <p class="card__text"><?= Security::e($card['text']) ?></p>
          <?php endif; ?>
        </li>
      <?php endforeach; ?>
    </ul>
