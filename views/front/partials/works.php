<?php
/**
 * Referanslar — portfoy kayitlarindan son N tanesi.
 * DOCS.md 5 (bolum 7)
 *
 * @var array $content
 * @var array $projects
  ...
  <div class="wrap">
        ...
        <li class="work" data-reveal>
          ...
          <h3 class="work__title">
            <a href="<?= Security::e(url('/referanslar/' . ($project['slug'] ?? ''))) ?>">
              <?= Security::e($project['title'] ?? $project['client_name'] ?? '') ?>
            </a>
          </h3>
    ...
    <?php if ($cta !== null && ($cta['label'] ?? '') !== ''): ?>
      <p class="section__more" data-reveal>
        <a class="btn btn--ghost" href="<?= Security::e(url((string) ($cta['url'] ?? '/referanslar'))) ?>">
          <?= Security::e($cta['label']) ?>
        </a>
      </p>
    <?php endif; ?>
  </div>
