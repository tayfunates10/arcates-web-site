...
test('U-15', 'Googlebot bot olarak işaretlenir', function (): void {
    ...
    assertSame('bot', $empty->device(), 'Boş tarayıcı imzası bot sayılmalı');
});

test('U-15b', 'Masaüstü, mobil ve tablet ayrımı', function (): void {
    ...
    $mobile = Request::make('GET', '/', [], [], [
        'HTTP_USER_AGENT' => 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Maç OS X) AppleWebKit/605 Mobile/15E148',
    ]);
    assertSame('mobile', $mobile->device());

    $tablet = Request::make('GET', '/', [], [], [
        'HTTP_USER_AGENT' => 'Mozilla/5.0 (iPad; CPU OS 17_0 like Maç OS X) AppleWebKit/605 Safari/604.1',
    ]);
    assertSame('tablet', $tablet->device());
});

test('U-15c', 'UTM parametreleri toplanır', function (): void {
    ...
    assertFalse(isset($utm['baska']), 'Beklenmeyen parametre alınmamalı');
});

test('U-15d', 'Yol normalleştirmesi', function (): void {
