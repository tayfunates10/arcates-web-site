...
final class RedirectController extends Controller
{
    ...
    /** Elle yonlendirme ekler veya gunceller. */
    public function store(Request $request, array $params): Response
    {
        ...
        if (!$result['ok']) {
            // Dongu kontrolu panelde engeller. DOCS.md 9.8, test S-18
        ...
        Logger::activity(
            ...
            'redirect',
            $id > 0 ? $id : null,
            $request->str('from_path') . ' → ' . $request->str('to_path')
        );
    ...
    /**
     * 404 kaydini tek tikla yonlendirmeye cevirir.  DOCS.md 9.8, test F-12
     */
    public function convert(Request $request, array $params): Response
    {
        ...
        // Donusturulen kayit 404 listesinden dusurulur.
    ...
    {
        return $this->db()->all(
            'SELECT * FROM not_found ORDER BY hits DESC, last_seen DESC LIMIT ' . self::PER_PAGE
        );
    }
}
